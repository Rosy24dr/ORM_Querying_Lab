...
def problem_one(request):
    students = Student.objects.all()
    descending_gpa = students.filter(gpa__gt =3.0).order_by('-gpa')
    for student in descending_gpa:
...
def problem_two(request):
  ...
  hire_date = instructor.filter(hire_date__year__lt = 2010).order_by('hire_date')

  for instructor in hire_date:
...
def problem_three(request):
  ...
  for course_one in courses:
    print(f'Full Name: {instructors.first_name} {instructors.last_name} Courses: {course_one.name}')
    return complete(request)
...
def problem_four(request):
  ...
  print(f'Students Count: {students} Instructors Count: {instructors} Courses Count: {courses}')
  return complete(request)
...
def problem_five(request):
    student = Student.objects.create(first_name='Rosy', last_name='Ferrell', year=2005, gpa=3.8)
    print(f'Id: {student.id} Full Name: {student.first_name} {student.last_name} Year: {student.year} GPA: {student.gpa}')
    return complete(request)
...
def problem_six(request):
    ...
    student_id = 11
    Student.objects.filter(id=student_id).update(gpa=2.6)
    student_one = Student.objects.get(id=student_id)
    print(f'Id: {student_one.id} Full Name: {student_one.first_name} {student_one.last_name} GPA: {student_one.gpa}')
    return complete(request)
